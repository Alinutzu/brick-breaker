...
(function(){
  ...
  soundBtn.addEventListener('click', async ()=>{
    ...
    if(audioCtx.state==='suspended' && audioEnabled){ try{ await audioCtx.resume(); }catch{} }
  });
  ...
  function circleRectCollide(cx,cy,r,rx,ry,rw,rh){
    const closestX=clamp(cx,rx,rx+rw); const closestY=clamp(cy,ry,ry+rh);
  ...
  function spawnParticles(x,y,color,count=8){
    for(let i=0;i<count;i++){
      const ang=randRange(0,Math.PI*2); const speed=randRange(40,160);
      particles.push({x,y,vx:Math.cos(ang)*speed,vy:Math.sin(ang)*speed,life:0.35,alpha:1,color});
    }
  }
  function updateParticles(dt){
    for(let i=particles.length-1;i>=0;i--){
      const p=particles[i]; p.x+=p.vx*dt; p.y+=p.vy*dt; p.vx*=0.92; p.vy*=0.92; p.life-=dt;
      p.alpha=Math.max(0,p.life/0.35); if(p.life<=0) particles.splice(i,1);
    }
  }
  ...
  class Paddle{
    ...
    update(dt,input){
      ...
      this.x+=this.vx*PADDLE_SPEED*dt; this.x=clamp(this.x,0,LOGICAL_W-this.w);
  ...
  class Ball{
    constructor(x,y,speedMul=1){
      this.x=x; this.y=y; const angle=randRange(Math.PI*0.25,Math.PI*0.75);
      const speed=INIT_BALL_SPEED*speedMul; this.vx=Math.cos(angle)*speed; this.vy=-Math.abs(Math.sin(angle)*speed);
      this.r=BALL_RADIUS; this.color='#f4f1c9'; this.pierceTimer=0; this.pierceMax=0;
    }
    update(dt){
      this.x+=this.vx*dt; this.y+=this.vy*dt;
      if(this.x-this.r<0){ this.x=this.r; this.vx=Math.abs(this.vx); sfx.bounce(); }
      if(this.x+this.r>LOGICAL_W){ this.x=LOGICAL_W-this.r; this.vx=-Math.abs(this.vx); sfx.bounce(); }
  ...
  class Brick{
    constructor(x,y,w,h,hp=1,requiresPierce=false,indestructible=false,opensCorridor=false){
      this.x=x; this.y=y; this.w=w; this.h=h; this.hp=hp; this.requiresPierce=requiresPierce; this.indestructible=indestructible; this.opensCorridor=opensCorridor;
    }
    draw(ctx){
      let c='#7aa2ff'; if(this.indestructible){ c='#9aa0a6'; } else if(this.hp>1){ c='#ff7b7b'; }
      if(this.requiresPierce) c='#ffa500'; if(this.opensCorridor) c='#00d1b2';
      ctx.fillStyle=c; ctx.fillRect(this.x,this.y,this.w,this.h); ctx.strokeStyle='#0c1229'; ctx.strokeRect(this.x+0.5,this.y+0.5,this.w-1,this.h-1);
    }
  }
  class PowerUp{
    constructor(x,y,type){ this.x=x; this.y=y; this.type=type; this.w=14; this.h=14; this.vy=140; this.active=true; }
  ...
  for(let i=1;i<=12;i++){
    const b=document.createElement('button'); b.className='levelbtn'; b.textContent='Nivel '+i + (i===12?' ★':'');
    b.addEventListener('click',()=>{ setMode('level'); state.level=i; resetGame(); hideMenu(); });
    grid.appendChild(b);
  }
  btnMenu.addEventListener('click',()=>{ showMenu(); });
  btnCloseMenu.addEventListener('click',()=>{ hideMenu(); });
  btnChooseLevel.addEventListener('click',()=>{ setMode('level'); resetGame(); hideMenu(); });
  ...
  window.addEventListener('keydown',(e)=>{
    ...
    if(k==='r'){
      ...
      if(state.mode==='level'){ buildLevel(); } else { state.wave=1; buildLevel(); }
      state.lives=3; state.balls=[new Ball(LOGICAL_W/2,LOGICAL_H-80,1)];
      state.running=true; uiLives.textContent=state.lives;
    }
  });
  ...
  canvas.addEventListener('touchstart', (e)=>{
    if(!useDrag) return; const t=e.changedTouches[0];
    const rect=canvas.getBoundingClientRect();
    const x=(t.clientX-rect.left)/rect.width*LOGICAL_W;
  ...
  function resetGame(){
    state.paddle=new Paddle(); state.balls=[new Ball(LOGICAL_W/2, LOGICAL_H-60, 1)];
    state.powerUps=[]; state.score=0; state.lives=3; state.stageCleared=false; state.wave=1;
    buildLevel(); state.running=true; uiStage.textContent=(state.mode==='endless')?state.wave:state.level; uiScore.textContent=state.score; uiLives.textContent=state.lives;
  }
  function nextStage(){
    if(state.mode==='level'){
      state.stageCleared=true; state.running=false;
    } else {
      state.wave+=1; buildLevel(); state.balls=[new Ball(LOGICAL_W/2, LOGICAL_H-60, 1)];
      state.stageCleared=false; state.running=true; uiStage.textContent=state.wave;
    }
  }
  ...
  function checkStalemateSuccess(){
    ...
    const pierceActive = state.balls.some(b=>b.pierceTimer>0);
    return !pierceActive;
  }
  ...
  function updateTimersHUD(){
    if(state.paddle.wideTimer>0){
      ...
      const pct = clamp(state.paddle.wideTimer / Math.max(0.0001, state.paddle.wideMax||8), 0, 1) * 100;
      wideFill.style.width = pct.toFixed(1)+'%';
    } else { wideBox.hidden=true; }

    let maxPierce=0,maxPierceMax=0;
    for(const b of state.balls){ if(b.pierceTimer>maxPierce){ maxPierce=b.pierceTimer; maxPierceMax=Math.max(maxPierceMax, b.pierceMax||maxPierce); } }
    if(maxPierce>0){
      pierceBox.hidden=false;
      const pct = clamp(maxPierce / Math.max(0.0001, maxPierceMax||5), 0, 1) * 100;
      pierceFill.style.width = pct.toFixed(1)+'%';
  ...
  function update(dt){
      ...
      if(collide){
        ...
        if(pu.type==='multiball'){
          ...
          for(const b of state.balls){
            if(state.balls.length+newBalls.length>=MAX_BALLS) break;
            const nb1=new Ball(b.x,b.y,1), nb2=new Ball(b.x,b.y,1);
          ...
          state.paddle.wideTimer=Math.max(state.paddle.wideTimer,8); state.paddle.wideMax=Math.max(state.paddle.wideMax,8);
        } else if(pu.type==='pierce'){
          for(const b of state.balls){ b.pierceTimer=Math.max(b.pierceTimer||0,5); b.pierceMax=Math.max(b.pierceMax||0,5); b.color='#ffc0cb'; }
        } else if(pu.type==='split'){
          const splitBalls=[]; const original=state.balls.slice(0,Math.min(state.balls.length,6));
          for(const b of original){
            if(state.balls.length+splitBalls.length>=MAX_BALLS) break;
            for(let i=0;i<2;i++){
              const nb=new Ball(b.x,b.y,1); const angle=randRange(-0.6,0.6); const speed=Math.hypot(b.vx,b.vy);
              nb.vx=Math.cos(angle)*speed; nb.vy=-Math.abs(Math.sin(angle)*speed); nb.color='#47e5bc';
              splitBalls.push(nb);
            }
          }
    ...
    for(const ball of state.balls){
      ...
      const padd=state.paddle.rect();
      if(circleRectCollide(ball.x,ball.y,ball.r,padd.x,padd.y,padd.w,padd.h) && ball.vy>0){
        const hitPos=(ball.x-padd.x)/padd.w-0.5; const angle=hitPos*Math.PI*0.7; const speed=Math.hypot(ball.vx,ball.vy);
        ball.vx=Math.sin(angle)*speed; ball.vy=-Math.abs(Math.cos(angle)*speed);
        ball.y=padd.y-ball.r-0.01; sfx.bounce();
      }
      ...
      for(let i=state.bricks.length-1;i>=0;i--){
        ...
        if(circleRectCollide(ball.x,ball.y,ball.r,br.x,br.y,br.w,br.h)){
          if(br.indestructible && !(ball.pierceTimer>0)){
            const dx=(ball.x-(br.x+br.w/2))/(br.w/2); const dy=(ball.y-(br.y+br.h/2))/(br.h/2);
            if(Math.abs(dx)>Math.abs(dy)) ball.vx=-ball.vx; else ball.vy=-ball.vy;
            sfx.bounce(); spawnParticles(ball.x,ball.y,'#9aa0a6',5); continue;
          }
          if(br.requiresPierce && !(ball.pierceTimer>0)){
            const dx=(ball.x-(br.x+br.w/2))/(br.w/2); const dy=(ball.y-(br.y+br.h/2))/(br.h/2);
            if(Math.abs(dx)>Math.abs(dy)) ball.vx=-ball.vx; else ball.vy=-ball.vy;
            sfx.bounce(); spawnParticles(ball.x,ball.y,'#ffa500',6); continue;
          }
          if(!(ball.pierceTimer>0)){
            const dx=(ball.x-(br.x+br.w/2))/(br.w/2); const dy=(ball.y-(br.y+br.h/2))/(br.h/2);
            if(Math.abs(dx)>Math.abs(dy)) ball.vx=-ball.vx; else ball.vy=-ball.vy; sfx.bounce();
          }
          if(!br.indestructible){
            br.hp-=1; uiScore.textContent=(state.score+=10); sfx.brick();
            spawnParticles(ball.x,ball.y, br.requiresPierce?'#ffa500':'#7aa2ff', 10);
            if(br.hp<=0){
              state.bricks.splice(i,1);
              if(br.opensCorridor && !state.corridor2Opened){
                state.corridor2Opened=true;
                Levels.openSecondCorridor({
                  bricks:state.bricks, barrierRows:state.barrierRows, offsetX:state.offsetX,
                  corridor2Col:state.corridor2Col, BRICK_W, BRICK_H, BRICK_GAP
                });
              }
      ...
      if(ball.y-ball.r>LOGICAL_H){
        ...
        if(state.balls.length===0){
          state.lives-=1; sfx.lose(); uiLives.textContent=state.lives;
          if(state.lives>0){
            buildLevel(); state.powerUps=[]; state.balls=[new Ball(LOGICAL_W/2,LOGICAL_H-80,1)];
            state.running=true; state.stageCleared=false; uiStage.textContent=(state.mode==='endless')?state.wave:state.level;
  ...
  function draw(){
    ...
    if(!state.running){
      ...
      const msg = (state.lives<=0) ? 'Game Over' : (state.stageCleared && state.mode==='level' ? 'Nivel complet!' : (input.paused ? 'Pauză' : ''));
      if(msg) ctx.fillText(msg, renderW/2, renderH/2 - 12);
      ctx.font='14px system-ui'; ctx.fillText('Apasă R pentru Reset sau deschide Meniul', renderW/2, renderH/2 + 18);
    }
  }
